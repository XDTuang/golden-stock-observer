...
NIGHT_SCRUB_RULES = [
    ...
    # ── 2026-09-04 追加：上一版正则过严导致漏网（`agent 投喂精选` 因 b 标签带属性未命中）
    (r'agent\s*投喂精选', '精选补充'),                               # 去掉标签限定，直接匹配文本
    (r'(持仓端)?\s*剑桥科技冲高减半、华工科技破\s*95\s*离场',          # 7.1 表 / 7.3 回避清单内的个人持仓指令
     '光模块链整体偏弱，暂不具备参与条件'),
    (r'改变周末持仓意愿', '改变周末持股意愿'),                        # 7.3 通用表述中性化
]
