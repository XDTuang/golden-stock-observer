...
// 🔴 2026-09-22 补齐（与老站 inject_feed_review.rich() / V3 patch_v3_richtext.rich() 同口径）：
//    ① 放行**带 class 的** <b|i|span class="dk-*">（**单双引号都认**，class 须 ⊆ OBS_DKCLS）
//    ② **解一层多余实体**（&amp;gt; → &gt;）—— agent 在 obs_panel.json 里写了 HTML 实体，
//       只做一次转义会让页面显示字面「&gt;」（同一文本在老站静态 HTML 里是正常的，故只在 JS 渲染侧需要）
//    ③ ⚠️ 顺序 = 先放行标签、再解实体；反序会把刻意的 `&lt;b&gt;` 误变成真标签
//    <script>/<img>/<a> 等仍全转义（无 XSS 面）
const OBS_DKCLS = /^dk-(main|caution|risk|data|up|dn|neutral)$/;
const _obsRtTag = (m, close, tag, dq, sq) => {
  if (close) return '</' + tag + '>';
  const cls = ((dq == null ? sq : dq) || '').trim();
  if (!cls || !cls.split(/\s+/).every(c => OBS_DKCLS.test(c))) return m;
  return '<' + tag + ' class="' + cls + '">';
};
const obsRich = (v) => String(v == null ? '' : v)
  .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  .replace(/&lt;(\/?)(b|i|em|strong|code|span)\s+class=(?:"([^"]*)"|'([^']*)')\s*\/?&gt;/g, _obsRtTag)
...
function obsL2(picks, byCode, scenDate) {
  ...
  picks.forEach(p => {
    ...
    rows.push('<details class="obs-fold"><summary>' +
      ...
      '<div class="obs-body">' + (p.reason ? '<div class="obs-rsn">入选理由：' + obsRich(p.reason) + '</div>' : '') + obsKl(x, k) +
